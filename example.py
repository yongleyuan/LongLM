...
# model_lists = ['google/gemma-7b-it', 'meta-llama/Llama-2-7b-chat-hf', 'mistralai/Mistral-7B-Instruct-v0.1', 'microsoft/phi-2']
model_lists = ["susnato/phi-2"]
...
for model_name in model_lists:
    ...
    for line in open(file_name, "r"):
        ...
        input_ids = input_ids.to(model.device) # Need this line to distribute weights to automatically assigned gpus
        print("-----------------------------------")
        ...
        end_time = time.time()
        answer = prompt_postfix + tokenizer.decode(
    ...
    SelfExtend.apply(
        ...
        flash_attention_impl="flash_attn", # flash_attention_impl="triton" or "flash_attn"
    )  
    for line in open(file_name, "r"):
        example = json.loads(line)
        prompt_postfix = "What is the pass key? The pass key is "
    ...
    # )  ## flash_attention_impl="triton" or "flash_attn"
    # for line in open(file_name, "r"):
    #     example = json.loads(line)
    #     prompt_postfix = "What is the pass key? The pass key is "
    #     prompt = example["input"] + prompt_postfix
